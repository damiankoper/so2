...
std::vector<Vector2i> Relation::getPoints() {
  ...
  for (size_t i = 0; i < stops.size() - 1; i++) {
    ...
    Stop *next = stops[i + 1];
    auto currentMatch =
        std::find(current->relations.begin(), current->relations.end(), this);
    auto nextMatch =
        std::find(next->relations.begin(), next->relations.end(), this);
    ...
    prevOutputDir = addPointsForPositions(
        points, current->position.add(Vector2i(0, currentStopNth * 15)),
        next->position.add(Vector2i(0, nextStopNth * 15)), prevOutputDir);
  }

  Stop *lastStop = stops[stops.size() - 1];
  auto stopMatch =
      std::find(lastStop->relations.begin(), lastStop->relations.end(), this);
  int stopNth = stopMatch - lastStop->relations.begin();
  points.push_back(
      stops[stops.size() - 1]->position.add(Vector2i(0, stopNth * 15)));
  return points;
}
...
Vector2i Relation::addPointsForPositions(std::vector<Vector2i> &points,
  ...
  // Horizontal same Y U-turn
    ...
    return addPointsForPositions(points,
                                 Vector2i(from.x, from.y - 3 * minShift),
                                 Vector2i(to.x, to.y), -prevOutputDirection);
  }
  // Horizontal curve
    ...
    // next in
    points.emplace_back(
        from.x + (minShift + abs(deltaY)) * prevOutputDirection.x, to.y);
  ...
  // Horizontal to horizontal vertical - U-turn
    ...
    if (deltaX * prevOutputDirection.x > 0) {
      ...
      // current out turn 1
      points.emplace_back(from.x + (2 * minShift * prevOutputDirection.x),
                          from.y + (minShift * sY));
...
float Relation::getStopDistance(Stop *targetStop) {
  ...
  if (!this->stopDistanceCache.contains(targetStop)) {
    ...
    auto match = std::find(targetStop->relations.begin(),
                           targetStop->relations.end(), this);
    int stopNth = match - targetStop->relations.begin();
    Vector2i realStopPosition =
        targetStop->position.add(Vector2i(0, 15 * stopNth));
