...
#include <QString>
void StopDrawer::setStop(Stop *stop) { this->stop = stop; }
...
void StopDrawer::draw(QPainter &painter) {
  ...
  std::string text =
      std::to_string(stop->passengers.size()) + "  " + stop->name;

  int centerDelta = STOP_SIZE / 2;
  QRect rect = QRect(
      stop->position.x - centerDelta, stop->position.y - centerDelta, STOP_SIZE,
      STOP_SIZE + std::max(((int)stop->relations.size() - 1) * 15, 0));
  painter.drawRect(rect);
  painter.drawText(stop->position.x - centerDelta - STOP_STROKE / 2,
                   stop->position.y - (centerDelta + STOP_STROKE),
                   QString(text.c_str()));
}